...
{
    public class FoxItemActor : ReceiveActor
    {
        ...
        protected override void PreStart()
        {
            ...
            var ids = Program.cdb.GetDocumentQuery("newsfeed", "select value c.id from c where c.partionKey = 'FOX' and c.pubDate > '" + dt + "'");
            var res = ids.ToList();
        ...
        public FoxItemActor()
        {
            List<FoxNewsItem> items = new List<FoxNewsItem>() ;

            var counterActor = Context.ActorSelection("akka://feed-system/user/wordCount");
            Receive<FoxNews.processFoxItem>(i =>
            {
                ...
                if (!idsProcessed.Contains(id))
                {
                    ...
                    if (d.IndexOf("<") != 0)
                    {
                        ...
                        var idx = item.link.IndexOf("/", 24);
                        item.siteSection = item.link.Substring(24, idx - 24);
                        ...
                        Context.Parent.Tell(new processedFox());
                }
                else
                    Context.Parent.Tell(new processedFox());
            });
            ...
            Receive<string>(r =>
            {
                ...
                else if (r.StartsWith("ner:"))
                {
                    ...
                    Self.Tell(new processedFox() { id = item.id  });
                }
            });
            ...
            Receive<processedFox>(r =>
            {
                ...
                Console.WriteLine("=========> Fox saving " + item.siteSection);

                counterActor.Tell(new NLPHelper.CountRequest()
                {
                    Feed = "Fox",
                    Id = item.id,
