...
namespace LiebFeed.NLPHelper
{
    ...
    /// </summary>
    public class StopwordActor : ReceiveActor
    {
        /// <summary>
        ...
        static Dictionary<string, bool> _stops = new Dictionary<string, bool>
    {
        ...
        {
            Receive<StopwordRequest>(s =>
            {
                if (s.linesOfText.Count > 1)
                {
                    var ret = new List<string>();
                    foreach (var i in s.linesOfText)
                    {
                        ret.Add(RemoveStopwords(i));
                    }
                    Sender.Tell(new StopwordResponse() { id = s.id, outputStrings = ret });
                }
                else
                {
                    Sender.Tell(new StopwordResponse()
                    {
                        id = s.id,
                        outputStrings = new List<string>() { RemoveStopwords(s.linesOfText.First()) }
                    });
                }
            });
        }
    ...
    {
        public StopwordRequest()
        {

        }
        public StopwordRequest(string inputWord)
        {
            linesOfText = new List<string>() { inputWord };
        }
